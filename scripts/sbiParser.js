...
export class sbiParser {
  static async parseInput(lines, selectedFolderId) {
    ...
    if (lines.length) {
      ...
      let resultArray = lines
        .map((l) => l.trim())
        .filter((item) => !foundLines.includes(item));
      ...
      for (let [key, value] of statBlocks.entries()) {
        switch (key) {
          ...
          case BlockID.damageVulnerabilities:
            this.setDamagesAndConditions(
              value,
              DamageConditionId.vulnerabilities,
              creature
            );
            break;
      ...
      console.log(creature.special_abilities_json)
      creature.special_abilities_json = this.actionBlockDataToJson(creature.special_abilities_json);
      creature.spells_json = this.actionBlockDataToJson(creature.spells_json);
  ...
  static setActions(lines, type, creature) {
    ...
    if (type === BlockID.villainActions) {
      creature.special_abilities_json.push(this.getVillainActions(lines));
      ...
      for (const actionData of this.getBlockDatas(lines)) {
        ...
        if (nameLower === "spellcasting") {
          this.getSpells(
            ...
            creature.spells_json.push(spellData);
        });;
        } else if (nameLower === "innate spellcasting") {
          this.getSpells(
            actionData.description,
            sRegex.spellInnateLine
          ).forEach((spellData) => {
            creature.spells_json.push(spellData);
        });;
        } else {
          featureDatas.push(
            new NameValueData(actionData.name, actionData.description)
          );
        }
      }

      for(let feature of featureDatas)
        creature.special_abilities_json.push(feature);
      ...
      if (spellDatas.length === 1) {
        ...
        if(type === BlockID.actions)
            creature.actions_json = this.actionBlockDataToJson(this.getBlockDatas(lines));
        else if(type === BlockID.bonusActions)
            creature.bonus_actions_json = this.actionBlockDataToJson(this.getBlockDatas(lines));
        else if(type === BlockID.lairActions){
            this.getBlockDatas(lines).forEach((actionData) => {
            creature.special_abilities_json.push(actionData);
            })
        }
        else if(type === BlockID.legendaryActions)
            creature.legendary_actions_json = this.actionBlockDataToJson(this.getBlockDatas(lines));
            ...
            creature.reactions_json = this.actionBlockDataToJson(this.getBlockDatas(lines));
        else if(type === BlockID.traits){
            this.getBlockDatas(lines).forEach((actionData) => {
            creature.special_abilities_json.push(actionData);
            })
        }
        else if(type === BlockID.villainActions){
            this.getBlockDatas(lines).forEach((actionData) => {
            creature.special_abilities_json.push(actionData);
            })
  ...
  static actionBlockDataToJson(actionBlockData) {
    const actions = {};

    for (const actionData of actionBlockData) {
        actions[actionData.name] = actionData.description;
    }
  ...
  static setArmor(lines, creature) {
    ...
    // AC value
    creature.armor_class = match.groups.ac;
    // Armor types, like "natural armor" or "leather armor, shield"
    creature.armor_desc = match.groups.armortype
      ?.split(",")
      .map((str) => str.trim());
  }
  ...
  static setAbilities(lines, creature) {
    ...
    for (let i = 0; i < foundAbilityNames.length; i++) {
      if (foundAbilityNames[i] === "STR") {
        creature.strength = parseInt(foundAbilityValues[i]);
      } else if (foundAbilityNames[i] === "DEX") {
        creature.dexterity = parseInt(foundAbilityValues[i]);
      } else if (foundAbilityNames[i] === "CON") {
        creature.constitution = parseInt(foundAbilityValues[i]);
      } else if (foundAbilityNames[i] === "INT") {
        creature.intelligence = parseInt(foundAbilityValues[i]);
      } else if (foundAbilityNames[i] === "WIS") {
        creature.wisdom = parseInt(foundAbilityValues[i]);
      } else if (foundAbilityNames[i] === "CHA") {
        creature.charisma = parseInt(foundAbilityValues[i]);
      }
    }
  }
  ...
  static setChallenge(lines, creature) {
    ...
    creature.challenge_rating = crValue;
  ...
  static setDamagesAndConditions(lines, type, creature) {
    ...
    switch (type) {
      case DamageConditionId.immunities:
        creature.damage_immunities = line;
        break;
      case DamageConditionId.resistances:
        creature.damage_resistances = line;
        break;
      case DamageConditionId.vulnerabilities:
        creature.damage_vulnerabilities = line;
        break;
      case BlockID.conditionImmunities:
        creature.condition_immunities = line;
        break;
    }
  ...
  static setRoll(lines, type, creature) {
    ...
    creature.hit_points = parseInt(match.groups.value);
    creature.hit_dice = match.groups.formula;
  }
  ...
  static setLanguages(lines, creature) {
    ...
    for (let index = 0; index < line.length; index++) {
      if (index > 0 && index < line.length - 1) {
        ...
        if (
          curLetter === "," &&
          !isNaN(Number(lastLetter) && !isNaN(nextLetter))
        ) {
    ...
    const values = modLine
      .split(",")
      .map((str) => str.toLowerCase().trim().replace("!", ","));
    const knownValues = sUtils.intersect(values, KnownLanguages);
    const unknownValues = sUtils
      .except(values, knownValues)
      .map((str) => sUtils.capitalizeFirstLetter(str));

    creature.languages = values
      .map((str) => sUtils.capitalizeFirstLetter(str))
      .join(", ");
  }
  ...
  static setSavingThrows(lines, creature) {
    ...
    const savesArray = line.split(",").map((item) => item.trim());

    savesArray.forEach((save) => {
      const parts = save.split("+").map((item) => item.trim());
      ...
      switch (ability.toLowerCase()) {
        case "str":
        case "strength":
          creature.strength_save = parseInt(value);
          break;
        case "dex":
        case "dexterity":
          creature.dexterity_save = parseInt(value);
          break;
        case "con":
        case "constitution":
          creature.constitution_save = parseInt(value);
          break;
        case "int":
        case "intelligence":
          creature.intelligence_save = parseInt(value);
          break;
        case "wis":
        case "wisdom":
          creature.wisdom_save = parseInt(value);
          break;
        case "cha":
        case "charisma":
          creature.charisma_save = parseInt(value);
          break;
        default:
          console.warn(`Invalid ability save: ${ability}`);
      }
    });
  }
  ...
  static setSenses(lines, creature) {
    ...
    const senses = matches.map(
      (m) => new NameValueData(m.groups.name, m.groups.modifier)
    );
    const sensesString = senses.map((sense) => sense.name + " " + sense.description).join(", ");
    creature.senses = sensesString;
  }

  static setSkills(lines, creature) {
    const line = sUtils.combineToString(lines);
    const matches = [...line.matchAll(sRegex.skillDetails)];
    const skills = matches.map(
      (m) => new NameValueData(m.groups.name, m.groups.modifier)
    );
    const convertedSkills = {};

    skills.forEach((skill) => {
      // Extract the name and description from each skill object
      const { name, description } = skill;
      // Extract the numeric value from the description string
      const value = parseInt(description, 10);
      // Add the converted data to the convertedSkills object
      convertedSkills[name] = value;
    });

    // Convert the object to a JSON string
    creature.skills_json = JSON.stringify(convertedSkills);
  }
  ...
  static setSpeed(lines, creature) {
    ...
    const speeds = match
      ...
      .filter((nv) => nv.name != null && nv.description != null);
    ...
    speeds.forEach((speed) => {
      speedsJson[speed.name] = speed.description;
    });

    creature.speed_json = JSON.stringify(speedsJson);
  }
  ...
  static setRacialDetails(lines, creature) {
    ...
    let singleCreatureType = creatureType.endsWith("s")
      ? creatureType.slice(0, -1)
      : creatureType;
  ...
  static getBlockDatas(lines) {
    ...
    for (let index = 0; index < validLines.length; index++) {
      ...
      if (!foundSpellBlock) {
        foundSpellBlock =
          line.match(/\binnate spellcasting\b|\bspellcasting\b/i) != null;
      ...
      if (foundSpellBlock && nextLineIsTitle) {
        ...
        if (!spellLines[spellLines.length - 1].endsWith(".")) {
          spellLines[spellLines.length - 1] =
            spellLines[spellLines.length - 1] + ".";
        }
    ...
    for (const sentence of sentences) {
      ...
      if (titleMatch && !foundTitle) {
        ...
        if (actionData == null) {
          ...
          result.push(actionData);
        } else if (actionData.description == null) {
          actionData.description = sentence;
        } else {
          actionData.description = `${actionData.description} ${sentence}`;
        }
      }
    }
    ...
    for (let index = 0; index < result.length; index++) {
      ...
      if (aData.description == null && index != 0) {
        ...
        result[index - 1].description =
          result[index - 1].description + " " + aData.name;
      } else {
        aData.description = this.formatForDisplay(aData.description);
      }
    }
  ...
  static getVillainActions(lines) {
    ...
    for (const line of lines) {
      ...
      if (titleMatch) {
        actionData = new NameValueData(
          titleMatch.groups.title,
          titleMatch.groups.description
        );
        result.push(actionData);
      } else {
        if (actionData == null) {
          ...
          actionData.description = `${actionData.description} ${line}`;
        }
      }
    }
  ...
  static getSpells(spellText, spellRegex) {
    ...
    if (spellMatches.length) {
      ...
      for (let idx = 0; idx < spellMatches.length; idx++) {
        ...
        let lastIndex =
          idx < spellMatches.length - 1
            ? spellMatches[idx + 1].index
            : undefined;
        ...
        spellGroups.push(
          new NameValueData(sUtils.trimStringEnd(match[0], ":"), spellNames)
        );
      }
    }
